...
class FakeOrder:
    def __init__(self) -> None:
        ...
        self.created_at = timezone.make_aware(datetime(2026, 1, 1, 8, 0, 0))
...
def test_get_admin_recipients_uses_configured_list() -> None:
    with patch.object(
        tasks.settings,
        "ADMIN_ORDER_ALERT_EMAILS",
        ["admin1@example.com", "", "admin1@example.com", "admin2@example.com"],
        create=True,
    ):
        with patch.object(tasks.settings, "EMAIL_HOST_USER", "", create=True):
            recipients = tasks._get_admin_recipients()

    assert recipients == ["admin1@example.com", "admin2@example.com"]


def test_get_admin_recipients_falls_back_to_email_host_user() -> None:
    with patch.object(tasks.settings, "ADMIN_ORDER_ALERT_EMAILS", [], create=True):
        with patch.object(tasks.settings, "EMAIL_HOST_USER", "mailer@example.com", create=True):
            recipients = tasks._get_admin_recipients()

    assert recipients == ["mailer@example.com"]


def test_get_admin_recipients_falls_back_to_staff_users() -> None:
    class FakeUserQuerySet:
        def __init__(self, emails: list[str]) -> None:
            self._emails = emails

        def exclude(self, **_kwargs: object) -> "FakeUserQuerySet":
            return self
    ...
    class FakeUserManager:
        def filter(self, **_kwargs: object) -> FakeUserQuerySet:
            return FakeUserQuerySet(["staff1@example.com", "staff1@example.com", "staff2@example.com"])
    ...
    assert recipients == ["staff1@example.com", "staff2@example.com"]


def test_send_new_order_admin_email_returns_no_admin_recipients() -> None:
    ...
    with patch.object(tasks, "_get_admin_recipients", return_value=[]):
        with patch.object(tasks, "send_mail") as send_mail_mock:
            result = tasks.send_new_order_admin_email.run(order_id=10)

    assert result == {"status": "no_admin_recipients", "order_id": 10}
    send_mail_mock.assert_not_called()


def test_send_new_order_admin_email_sends_to_admins() -> None:
    class FakeOrderModel:
        DoesNotExist = type("DoesNotExist", (Exception,), {})
        objects = FakeOrderQuery(order=FakeOrder())
    ...
    with patch.object(tasks, "_get_admin_recipients", return_value=["admin@example.com"]):
        with patch.object(tasks, "send_mail") as send_mail_mock:
            result = tasks.send_new_order_admin_email.run(order_id=11)

    assert result["status"] == "sent"
    assert result["order_number"] == "ODR-1001"
    assert result["recipients"] == ["admin@example.com"]
    send_mail_mock.assert_called_once()


def test_send_new_order_admin_email_handles_order_not_found() -> None:
    ...
    with patch.object(tasks, "_get_admin_recipients", return_value=["admin@example.com"]):
        result = tasks.send_new_order_admin_email.run(order_id=12)

    assert result == {"status": "order_not_found", "order_id": 12}


def test_send_new_order_admin_email_retries_on_unexpected_error() -> None:
    class FakeOrderModel:
        DoesNotExist = type("DoesNotExist", (Exception,), {})
        objects = FakeOrderQuery(error=ValueError("db timeout"))
    ...
    with patch.object(tasks.send_new_order_admin_email, "retry", retry_mock):
        with patch.object(tasks, "_get_admin_recipients", return_value=["admin@example.com"]):
            with pytest.raises(RuntimeError, match="retry called"):
                tasks.send_new_order_admin_email.run(order_id=13)
...
def test_send_order_status_email_sends_update_to_customer() -> None:
    class FakeOrderWithStatus(FakeOrder):
        def get_status_display(self) -> str:
            return "Dang giao"

    class FakeOrderModel:
        DoesNotExist = type("DoesNotExist", (Exception,), {})
        objects = FakeOrderQuery(order=FakeOrderWithStatus())
